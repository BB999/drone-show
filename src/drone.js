...
export function loadDroneModel() {
  ...
  // MU同時配置の位置（50機用）- Mが左、Uが右
  const muPositions = [];
  // M（25機）- 左側に配置
  const mOffsetX = -0.18;
  // M左縦線（7機）
  for (let i = 0; i < 7; i++) {
    muPositions.push({ x: mOffsetX - 0.08, y: 0.12 - i * 0.04, z: 0 });
  }
  // M左斜め（5機）
  for (let i = 0; i < 5; i++) {
    const t = i / 4;
    muPositions.push({ x: mOffsetX - 0.06 + t * 0.06, y: 0.12 - t * 0.12, z: 0 });
  }
  // M右斜め（5機）
  for (let i = 0; i < 5; i++) {
    const t = i / 4;
    muPositions.push({ x: mOffsetX + t * 0.06, y: 0.0 + t * 0.12, z: 0 });
  }
  // M右縦線（8機）
  for (let i = 0; i < 8; i++) {
    muPositions.push({ x: mOffsetX + 0.08, y: 0.12 - i * 0.04, z: 0 });
  ...
  // U左縦線（9機）
  for (let i = 0; i < 9; i++) {
    muPositions.push({ x: uOffsetX - 0.10, y: 0.14 - i * 0.032, z: 0 });
  }
  // U下曲線（7機）
  for (let i = 0; i < 7; i++) {
    const t = i / 6;
    const angle = Math.PI + t * Math.PI;
    muPositions.push({ x: uOffsetX + Math.cos(angle) * 0.10, y: -0.16 + Math.sin(angle) * 0.02, z: 0 });
  }
  // U右縦線（9機）
  for (let i = 0; i < 9; i++) {
    muPositions.push({ x: uOffsetX + 0.10, y: -0.14 + i * 0.032, z: 0 });
  ...
  // 上の横線（16機）
  for (let i = 0; i < 16; i++) {
    iPositions.push({ x: -0.18 + i * 0.024, y: 0.16, z: 0 });
  }
  // 縦線（18機）
  for (let i = 0; i < 18; i++) {
    iPositions.push({ x: 0, y: 0.13 - i * 0.016, z: 0 });
  }
  // 下の横線（16機）
  for (let i = 0; i < 16; i++) {
    iPositions.push({ x: -0.18 + i * 0.024, y: -0.16, z: 0 });
  ...
  // 左目 ^ - 4機
  smilePositions.push({ x: -0.08, y: 0.06, z: 0 });
  smilePositions.push({ x: -0.06, y: 0.10, z: 0 });
  smilePositions.push({ x: -0.04, y: 0.06, z: 0 });
  smilePositions.push({ x: -0.06, y: 0.08, z: 0 });
  // 右目 ^ - 4機
  smilePositions.push({ x: 0.04, y: 0.06, z: 0 });
  smilePositions.push({ x: 0.06, y: 0.10, z: 0 });
  smilePositions.push({ x: 0.08, y: 0.06, z: 0 });
  ...
  // === Xボタン用フォーメーション（猫 → メビウス → 泣き顔 → 波）===

  // 猫の配置位置（50機用）- 尻尾は別途アニメーション
  const catPositions = [];
  // 顔の輪郭（円形）- 14機
  for (let i = 0; i < 14; i++) {
    ...
    catPositions.push({
      ...
      y: Math.sin(angle) * 0.10 + 0.08,
      z: 0
    });
  }
  // 左耳（三角）- 4機
  catPositions.push({ x: -0.10, y: 0.16, z: 0 });
  catPositions.push({ x: -0.14, y: 0.22, z: 0 });
  catPositions.push({ x: -0.08, y: 0.22, z: 0 });
  catPositions.push({ x: -0.11, y: 0.19, z: 0 });
  // 右耳（三角）- 4機
  catPositions.push({ x: 0.10, y: 0.16, z: 0 });
  catPositions.push({ x: 0.14, y: 0.22, z: 0 });
  catPositions.push({ x: 0.08, y: 0.22, z: 0 });
  catPositions.push({ x: 0.11, y: 0.19, z: 0 });
  // 目（丸）- 左2機、右2機
  catPositions.push({ x: -0.05, y: 0.10, z: 0 });
  catPositions.push({ x: -0.04, y: 0.09, z: 0 });
  catPositions.push({ x: 0.05, y: 0.10, z: 0 });
  catPositions.push({ x: 0.04, y: 0.09, z: 0 });
  // 鼻（1機）
  catPositions.push({ x: 0, y: 0.05, z: 0 });
  // 口（W字）- 3機
  catPositions.push({ x: -0.03, y: 0.02, z: 0 });
  catPositions.push({ x: 0, y: 0.00, z: 0 });
  catPositions.push({ x: 0.03, y: 0.02, z: 0 });
  // ひげ（左3機、右3機）
  catPositions.push({ x: -0.16, y: 0.08, z: 0 });
  catPositions.push({ x: -0.17, y: 0.06, z: 0 });
  catPositions.push({ x: -0.16, y: 0.04, z: 0 });
  catPositions.push({ x: 0.16, y: 0.08, z: 0 });
  catPositions.push({ x: 0.17, y: 0.06, z: 0 });
  catPositions.push({ x: 0.16, y: 0.04, z: 0 });
  // 体（楕円）- 6機
  ...
  // メビウスの輪（50機用）- 八の字（∞）配置、循環アニメーション
  const mobiusPositions = [];
  for (let i = 0; i < 50; i++) {
    const t = (i / 50) * Math.PI * 2;
    // 八の字（レムニスケート）のパラメトリック方程式
    const a = 0.20; // サイズ
    const denom = 1 + Math.sin(t) * Math.sin(t);
    mobiusPositions.push({
      x: a * Math.cos(t) / denom,
      y: a * Math.sin(t) * Math.cos(t) / denom,
      z: 0,
      mobiusT: t // 循環アニメーション用
    });
  }

  // 泣き顔 (;_;) 配置の位置（50機用）
  const cryingPositions = [];
  // 顔の輪郭（円形）- 20機
  for (let i = 0; i < 20; i++) {
    const angle = (i / 20) * Math.PI * 2;
    cryingPositions.push({
  ...
  // 左目 ; （セミコロン）- 3機
  cryingPositions.push({ x: -0.07, y: 0.08, z: 0 }); // 点
  cryingPositions.push({ x: -0.07, y: 0.04, z: 0 }); // コンマ上
  cryingPositions.push({ x: -0.08, y: 0.02, z: 0 }); // コンマ下
  // 右目 ; - 3機
  cryingPositions.push({ x: 0.07, y: 0.08, z: 0 });
  cryingPositions.push({ x: 0.07, y: 0.04, z: 0 });
  cryingPositions.push({ x: 0.08, y: 0.02, z: 0 });
  // 口 _ - 4機
  cryingPositions.push({ x: -0.06, y: -0.06, z: 0 });
  cryingPositions.push({ x: -0.02, y: -0.06, z: 0 });
  cryingPositions.push({ x: 0.02, y: -0.06, z: 0 });
  cryingPositions.push({ x: 0.06, y: -0.06, z: 0 });
  // 涙（左）- 5機 - アニメーションで流れる
  for (let i = 0; i < 5; i++) {
  ...
  // 眉毛（困り眉）- 左2機、右2機
  cryingPositions.push({ x: -0.10, y: 0.12, z: 0 });
  cryingPositions.push({ x: -0.05, y: 0.14, z: 0 });
  cryingPositions.push({ x: 0.05, y: 0.14, z: 0 });
  cryingPositions.push({ x: 0.10, y: 0.12, z: 0 });
  // ほっぺ - 4機
  cryingPositions.push({ x: -0.13, y: -0.02, z: 0 });
  cryingPositions.push({ x: -0.14, y: -0.03, z: 0 });
  cryingPositions.push({ x: 0.13, y: -0.02, z: 0 });
  cryingPositions.push({ x: 0.14, y: -0.03, z: 0 });
  ...
  const wavePositions = [];
  for (let i = 0; i < 50; i++) {
    const t = (i / 49) * Math.PI * 4; // 2周期
    wavePositions.push({
      x: -0.24 + (i / 49) * 0.48,
      y: Math.sin(t) * 0.12,
      z: 0,
      wavePhase: t // 波アニメーション用
    });
  }
  ...
  state.setDroneMUPositions(muPositions);
  state.setDroneIPositions(iPositions);
  state.setDroneSmilePositions(smilePositions);
  // Xボタン用（猫 → メビウス → 泣き顔 → 波）
  state.setDroneCatPositions(catPositions);
  state.setDroneMobiusPositions(mobiusPositions);
  state.setDroneCryingPositions(cryingPositions);
  state.setDroneWavePositions(wavePositions);
